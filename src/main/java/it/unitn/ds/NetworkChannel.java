...
/**
 * A channel actor that delivers messages to a fixed destination with random delay, but preserving FIFO order. One instance per (sender, destination) pair.
 */
public class NetworkChannel extends AbstractActor {
  ...
  public Receive createReceive() {
    return receiveBuilder().match(Deliver.class, msg -> onDeliver()).matchAny(msg -> onEnqueue(msg, getSender())).build();
  }
  ...
  private void scheduleNextDelivery() {
    ...
    getContext().system().scheduler().scheduleOnce(Duration.create(delay, TimeUnit.MILLISECONDS), getSelf(), new Deliver(), getContext().system().dispatcher(), getSelf());
  }
