...
class NoCrashes {
  ...
  void sequentialConsistencyOneWriteClient(int coordinator, int n_nodes) throws InterruptedException {
    ...
    ActorRef writeClient = sys.system.actorOf(Client.propsWithListener(sys.client_read_timeout, sys.client_write_timeout, Optional.ofNullable(sys.actors.get(0)), null), "clientWrite");

    // Create read clients with read probe
    List<ActorRef> readClients = new ArrayList<>();
    for (int i = 0; i < sys.actors.size(); i++) {
      ActorRef readClient = sys.system.actorOf(Client.propsWithListener(sys.client_read_timeout, sys.client_write_timeout, Optional.ofNullable(sys.actors.get(i)), readProbe.getRef()), "clientRead_replica_" + i);
    ...
    for (Map.Entry<Integer, List<Integer>> entry : replicaValues.entrySet()) {
      ...
      for (int i = 1; i < values.size(); i++) {
        assertTrue(values.get(i - 1) == null || values.get(i) >= values.get(i - 1), String.format("Sequential consistency violated for replica %d: value at index %d (%d) is less than previous value (%d)", replicaId, i, values.get(i), values.get(i - 1)));
      }
    }
  }
