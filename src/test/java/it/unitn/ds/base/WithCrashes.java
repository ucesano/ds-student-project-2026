...
/**
 * IMPORTANT: Before running these tests, make sure you comply with the API and callback rules. Check out the java doc and tests in `APICompliance.java`.
 */
class WithCrashes {
  ...
  void nonCoordinatorsCrashClientWritesWaitsReads(int n_nodes) throws InterruptedException {
    ...
    final TestsSystemWrapper sys = TestsCommons.createTestSystem("nonCoordinatorsCrashClientWritesWaitsReads_" + COORDINATOR_ID, n_nodes, COORDINATOR_ID);
    TestKit probe = new TestKit(sys.system);
    int targetReplicaID = n_nodes - 1;
    ActorRef client = sys.system.actorOf(Client.propsWithListener(sys.client_read_timeout, sys.client_write_timeout, Optional.ofNullable(sys.actors.get(targetReplicaID)), probe.getRef()), "client");
    ...
    WriteResult wr = (WriteResult) probe.fishForMessage(Duration.ofMillis(TestsCommons.getMaxUpdateDelay(sys)), "WriteResult", msg -> msg instanceof WriteResult);
    ...
    ReadResult rr = (ReadResult) probe.fishForMessage(Duration.ofMillis(sys.client_read_timeout), "ReadResult", msg -> msg instanceof ReadResult);
  ...
  void coordinatorCrashClientWritesWaitsReads(int coordinator, int n_nodes) throws InterruptedException {
    ...
    client.tell(new AbstractClient.WriteRequest(TestsCommons.TEST_INDEX, TestsCommons.TEST_VALUE), Actor.noSender());
    WriteResult wr = (WriteResult) probe.fishForMessage(Duration.ofMillis(TestsCommons.getElectionMaxDelay(sys) + TestsCommons.getMaxUpdateDelay(sys)), "WriteResult", msg -> msg instanceof WriteResult);
    assertEquals(new WriteResult(true, TestsCommons.TEST_INDEX, TestsCommons.TEST_VALUE, targetReplicaID), wr);

    client.tell(new AbstractClient.ReadRequest(TestsCommons.TEST_INDEX), Actor.noSender());
    ReadResult rr = (ReadResult) probe.fishForMessage(Duration.ofMillis(TestsCommons.getLatencyPlusEpsilon(sys)), "ReadResult", msg -> msg instanceof ReadResult);
    assertEquals(new ReadResult(true, TestsCommons.TEST_INDEX, TestsCommons.TEST_VALUE, targetReplicaID), rr);
