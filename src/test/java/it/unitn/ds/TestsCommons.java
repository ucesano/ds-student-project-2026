...
/**
 * NOTE: ALL TIMINGS ON THESE CLASS MAY BE CHANGED. Make sure your implementation is robust and performant enough.
 */
public class TestsCommons {
  ...
  public static TestsSystemWrapper createTestSystem(String name, int n_actors, int coordinator, int min_latency, int max_latency) {
    ...
    for (int i = 0; i < n_actors; i++) {
      ...
      probes.put(i, probe);
      group.put(i, system.actorOf(Replica.propsWithListener(i, min_latency, max_latency, TEST_COORDINATOR_BEAT_INTERVAL, probe.getRef()), "Replica_" + i));
    }
  ...
  public static long getElectionMaxDelay(TestsSystemWrapper sys) {
    ...
    long detection = (long) (TEST_COORDINATOR_BEAT_INTERVAL * 3.0) + ((long) sys.max_latency * sys.getNNodes() * 2);
  ...
  public static long getClientReadTimeout(int max_lat, int n_nodes) {
    return (long) max_lat * n_nodes * 8;
  }

  public static long getClientWriteTimeout(int max_lat, int n_nodes) {
    long detection = (long) (TEST_COORDINATOR_BEAT_INTERVAL * 3.0) + ((long) max_lat * n_nodes * 2);
  ...
  public static class TestsSystemWrapper {
    ...
    public TestsSystemWrapper(ActorSystem system, Map<Integer, ActorRef> actors, Map<Integer, TestKit> probes, int min_latency, int max_latency) {
      this.system = system;
      this.actors = actors;
      this.probes = probes;
